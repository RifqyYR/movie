<form action="{{ Auth::user()->role == 'FINANCE' ? route('claim.approve.finance') : route('claim.approve.head') }}"
    method="POST">
...
if (!$claims->isEmpty()) {
      echo 'id="my-table"';
        ...
        <thead>
            <tr>
                <th scope="col" class="text-center align-middle custom-col"></th>
        ...
        <tbody>
            @php
                ...
                $allowedStaffEditStatus = [
                    App\Models\Claim::STATUS_TELAH_REGISTER_BOA,
                    App\Models\Claim::STATUS_TELAH_SETUJU,
                ];
            @endphp
                    ...
                    <tr
                        ...
                        <td class="text-center align-middle">
                            @if (Auth::user()->role == 'FINANCE')
                                <input type="checkbox" id="checkbox{{ $item->id }}" name="ids[]"
                                    value="{{ $item->uuid }}" name="ids[]"
                        ...
                        <td class="align-middle fw-bold text-nowrap table-custom-width table-custom-fs">
                            {{ $item->claim_type }} </td>
                        <td class="align-middle text-center fw-bold text-nowrap table-custom-width table-custom-fs">
                            {{ $item->month }} </td>
                        ...
                        <td class="align-middle fw-bold lh-1 table-custom-fs">
                            @php
                                if ($item->status == App\Models\Claim::STATUS_TELAH_SETUJU) {
                                    ...
                                    if (
                                        (auth()->user()->role == 'VERIFICATOR' || auth()->user()->role == 'ADMIN') &&
                                        (strpos(strtolower($item->claim_type), 'ambulance') !== false ||
                                            strpos(strtolower($item->claim_type), 'alkes') !== false)
                                    ) {
                                        if (
                                            $item->status == App\Models\Claim::STATUS_BA_SERAH_TERIMA ||
                                            $item->status == App\Models\Claim::STATUS_BA_KELENGKAPAN_BERKAS ||
                                            $item->status == App\Models\Claim::STATUS_BA_HASIL_VERIFIKASI
                                        ) {
                                            echo '<a class="text-black link-status" href="#" id="badropdown" role="button" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                                                            <span class="fw-bold" style="0.7rem;">' .
                                                $item->status .
                        ...
                        @if (auth()->user()->role != 'GUEST')
                            <td class="align-middle fw-bold action-col">
                                            ...
                                            <div>
                                                <div class="input-group">
                                                    ...
                                                    <input id="input-no-reg-fpk-ri" type="text"
                                                        ...
                                                        name="no_reg_fpk_ri" placeholder="Masukkan Nomor FPK RI"
                                                        autofocus autocomplete="no_reg_fpk_ri" />
                                                </div>
                                            </div>
                                            <div>
                                                <div class="input-group">
                                                    ...
                                                    <input id="input-no-reg-fpk-rj" type="text"
                                                        ...
                                                        name="no_reg_fpk_rj" placeholder="Masukkan Nomor FPK RJ"
                                                        autofocus autocomplete="no_reg_fpk_rj" />
                                                </div>
                                            </div>
                                            <span id="fpk-reg-number-warning"
                                                class="text-danger table-custom-fs-larger">*
                                                Nomor RI
                                                atau
